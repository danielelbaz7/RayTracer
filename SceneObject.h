...
struct SceneObject {
    ...
    SceneObject(const Vector3 &center, uint32_t color, float dif, float spec, int shiny, float refl)
        : center(center), color(color), diffuseCoefficient(dif), specularCoefficient(spec), shininess(shiny), reflectivity(refl) {};
...
struct Sphere : SceneObject {
    ...
    Sphere(double r, const Vector3 &c, uint32_t co, float dif, float spec, int shiny, float refl) : SceneObject(c, co, dif, spec, shiny, refl), radius(r) {};
...
struct Plane : SceneObject {
    ...
    Plane(const Vector3 &c, const Vector3 &n, float length, float width, uint32_t co, float dif, float spec, int shiny, float refl)
        : SceneObject(c, co, dif, spec, shiny, refl), normalVector(normalize(n)), length(length), width(width) {
